...
1. **Given** the user is on the Welcome page, **When** they look at the top of the screen, **Then** they see a horizontal header bar with the app title ("Propo!") on the left and actions on the right, matching the Main page header pattern.
...
- **FR-002**: The app title ("Propo!") MUST use the same font size, font weight, and color on both pages.
- **FR-002a**: On the WelcomePage (no active session), the header bar MUST show only the app title "Propo!" on the left side, with no avatar or greeting. On the MainPage (active session), the header bar shows the avatar, greeting, and app title.
...
- Q: What should the WelcomePage header bar show on the left side (where MainPage shows avatar + greeting)? → A: Show only the app title "Propo!" on the left (no avatar/greeting since no session exists).
...
- The app title "Propo!" should remain visible on both pages; on the MainPage the title will appear in the horizontal bar alongside the existing avatar, greeting, and actions.
