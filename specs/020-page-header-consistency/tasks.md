...
**Independent Test**: Open the app as a new user (WelcomePage) — see horizontal header bar with "Propo!" left, language switcher right. Start a session (MainPage) — see the same bar height with "Propo!" + avatar + greeting left, language switcher + switch button right. Navigate between pages — no vertical content shift.
...
- [x] T007 [US1] Update MainPage tests to verify app title "Propo!" appears in the header bar in `frontend/tests/pages/MainPage.test.tsx`
