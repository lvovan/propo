...
- [ ] CHK011 - Is the question generation pool clearly defined with examples for each question type (percentage, ratio, fraction, rule-of-three)? [Clarity, Spec §FR-002]
- [ ] CHK012 - Is "response time" precisely defined — does it start on question render completion, DOM paint, or component mount? [Clarity, Spec §FR-007]
...
- [ ] CHK014 - Is the "10 primary rounds" count consistent across all documents — spec FR-001, data model (rounds array length 10), and formula generator (generate 10)? [Consistency, Spec §FR-001]
...
## Question Generation Requirements Quality

- [ ] CHK022 - Are the question pools for each category (percentage, ratio, fraction, ruleOfThree) explicitly documented with value constraints? [Completeness, Spec §FR-002]
...
- [ ] CHK025 - Are the display formats for each question type clearly defined (percentage: "A% of B = ?", ratio: "A : B = C : ?", etc.)? [Completeness, Spec §FR-002]
...
- [ ] CHK029 - Is it specified whether replay rounds show the player's previous incorrect answer or present the question fresh with no prior-answer context? [Gap]
