...
/**
 * Returns the correct answer for a formula.
 */
export function getCorrectAnswer(formula: Formula): number {
  return formula.correctAnswer;
}
 ...
 * Captures question type, values, correctness, and elapsed time.
...
export function extractRoundResults(rounds: Round[]): RoundResult[] {
  return rounds.slice(0, 10).map((round) => ({
    type: round.formula.type,
    values: round.formula.values,
