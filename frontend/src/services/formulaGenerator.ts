 ...
 *
 * Distribution: 5 Pure Numeric (balanced) + 5 Story Challenge (1 each sub-type + 2 random).
 * Order is shuffled.
 */
 ...
 * Biases numeric slots toward challenging numeric types and story slots toward challenging story types.
 */
export function generateImproveFormulas(
  ...
  // Build 5 numeric slots
  const numericSlots: QuestionType[] = buildBiasedSlots(
    PURE_NUMERIC_TYPES,
    challengeNumeric.map((i) => i.type),
    5,
    randomFn,
  );

  // Build 5 story slots (at least 1 of each guaranteed)
  const storySlots: QuestionType[] = buildBiasedStorySlots(
    challengeStory.map((i) => i.type),
    randomFn,
  );
...
function buildBiasedSlots(
  allTypes: QuestionType[],
  challengeTypes: QuestionType[],
  totalSlots: number,
  randomFn: () => number,
): QuestionType[] {
  // Start with 1 of each type
  const slots: QuestionType[] = [...allTypes];
  const remaining = totalSlots - slots.length;

  // Fill remaining with challenging types (or random if none)
  const uniqueChallenges = [...new Set(challengeTypes)];
  for (let i = 0; i < remaining; i++) {
    if (uniqueChallenges.length > 0) {
      slots.push(pickRandom(uniqueChallenges, randomFn));
    } else {
      slots.push(pickRandom(allTypes, randomFn));
    }
  }
...
function buildBiasedStorySlots(
  challengeTypes: QuestionType[],
  randomFn: () => number,
): QuestionType[] {
  // Guarantee 1 of each sub-type
  const slots: QuestionType[] = ['multiItemRatio', 'percentageOfWhole', 'complexExtrapolation'];
  const uniqueChallenges = [...new Set(challengeTypes)];

  // 2 remaining slots biased toward challenging types
  for (let i = 0; i < 2; i++) {
    if (uniqueChallenges.length > 0) {
      slots.push(pickRandom(uniqueChallenges, randomFn));
    } else {
      slots.push(pickRandom(STORY_CHALLENGE_TYPES, randomFn));
    }
  }
