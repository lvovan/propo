...
- Browser localStorage (`propo_players` key) and sessionStorage (`propo_session` key) (003-home-screen-rework)
...
- Browser localStorage (`propo_players` key, schema v3 → v4) and sessionStorage (`propo_session` key) (006-score-display)
...
- localStorage (`propo_players` key), sessionStorage (tab-scoped sessions) (012-improve-game-mode)
...
- localStorage (`propo_lang` key for language preference) (014-multilingual-support)
- TypeScript ~5.9.3, React 19.2.0 + react, react-dom, react-router-dom (custom i18n — no library) (015-portuguese-language)
- localStorage (`propo_lang` key — existing, unchanged) (015-portuguese-language)
...
- TypeScript 5.9, React 19.2 + `microsoft-clarity` (npm package), Vite 7.3, Vitest 4 (018-rename-to-propo, 019-clarity-telemetry)
- N/A (telemetry is fire-and-forget to external service) (019-clarity-telemetry)
- Browser localStorage (`propo_players`, `propo_lang`) and sessionStorage (`propo_session`) (018-rename-to-propo)
...
- Browser localStorage (key `propo_players`) (022-player-name-limit)
...
- TypeScript ~5.9.3, React 19.2 + Vite 7.3, React Router DOM 7.13, CSS Modules (029-story-challenge)
- Browser localStorage (`propo_players`), sessionStorage (`propo_session`) (029-story-challenge)
